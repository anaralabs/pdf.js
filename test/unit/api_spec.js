...
describe("api", function () {
  ...
  function getFirstImageOpIndex(operatorList) {
    for (let i = 0, ii = operatorList.fnArray.length; i < ii; i++) {
      const fn = operatorList.fnArray[i];
      if (fn === OPS.paintInlineImageXObject || fn === OPS.paintImageXObject) {
        return i;
      }
    }
    return -1;
  ...
  async function getImageDataAtIndex(operatorList, page, index) {
    const fn = operatorList.fnArray[index];
    if (fn === OPS.paintInlineImageXObject) {
      return operatorList.argsArray[index][0];
    }
    if (fn === OPS.paintImageXObject) {
      const [objId] = operatorList.argsArray[index];
  ...
  const INVERT_TEXT_BG = 23 / 255;
  const INVERT_TEXT_FG = 232 / 255;
  ...
  function invertRgbExpected(r, g, b) {
    let luma = 0.2126 * (r / 255) + 0.7152 * (g / 255) + 0.0722 * (b / 255);
    luma = clamp01(luma);
    luma = 1 - luma;
    ...
    const mapped = INVERT_TEXT_BG + (INVERT_TEXT_FG - INVERT_TEXT_BG) * luma;
    const out = toByte(mapped * 255);
    return [out, out, out];
  }
...
Caron Broadcasting, Inc., an Ohio corporation (“Lessee”).`)
    ...
    it("inverts 1bpp image data when requested", async function () {
      const loadingTask = getDocument(
        buildGetDocumentParams("images_1bit_grayscale.pdf", {
          isOffscreenCanvasSupported: false,
        })
      );
      const pdfDoc = await loadingTask.promise;
      ...
      const opListNormal = await pdfPage.getOperatorList({
        invertImages: false,
      });
      const imgIndex = getFirstImageOpIndex(opListNormal);
      expect(imgIndex).toBeGreaterThanOrEqual(0);
      const imgDataNormal = await getImageDataAtIndex(
        opListNormal,
        pdfPage,
        imgIndex
      );
      expect(imgDataNormal).not.toBeNull();
      expect(imgDataNormal.kind).toEqual(ImageKind.GRAYSCALE_1BPP);

      const opListInverted = await pdfPage.getOperatorList({
        invertImages: true,
      });
      const imgIndexInverted = getFirstImageOpIndex(opListInverted);
      expect(imgIndexInverted).toEqual(imgIndex);
      const imgDataInverted = await getImageDataAtIndex(
        opListInverted,
        pdfPage,
        imgIndexInverted
      );
      expect(imgDataInverted).not.toBeNull();

      expect(imgDataInverted).not.toBe(imgDataNormal);
      expect(imgDataInverted.kind).toEqual(imgDataNormal.kind);
      expect(imgDataInverted.data.length).toEqual(imgDataNormal.data.length);

      const data = imgDataNormal.data;
      const dataInv = imgDataInverted.data;
      const sampleLength = Math.min(32, data.length);

      for (let i = 0; i < sampleLength; i++) {
        expect(dataInv[i]).toEqual(data[i] ^ 0xff);
      }

      await loadingTask.destroy();
    });

    it("inverts RGB(A) image data using the dark theme mapping", async function () {
      const loadingTask = getDocument(
        buildGetDocumentParams("cmykjpeg.pdf", {
          isOffscreenCanvasSupported: false,
        })
      ...
      expect(imgDataNormal).not.toBeNull();
      expect(
        imgDataNormal.kind === ImageKind.RGB_24BPP ||
          imgDataNormal.kind === ImageKind.RGBA_32BPP
      ).toEqual(true);
      ...
      const data = imgDataNormal.data;
      const dataInv = imgDataInverted.data;
      const maxPixels = imgDataNormal.kind === ImageKind.RGBA_32BPP ? 8 : 10;

      if (imgDataNormal.kind === ImageKind.RGB_24BPP) {
        const pixelCount = Math.min(maxPixels, data.length / 3);
        for (let i = 0; i < pixelCount; i++) {
          const offset = i * 3;
          const [expectedR, expectedG, expectedB] = invertRgbExpected(
            data[offset],
            data[offset + 1],
            data[offset + 2]
          );
          expect(dataInv[offset]).toEqual(expectedR);
          expect(dataInv[offset + 1]).toEqual(expectedG);
          expect(dataInv[offset + 2]).toEqual(expectedB);
        }
      } else {
        const pixelCount = Math.min(maxPixels, data.length / 4);
        for (let i = 0; i < pixelCount; i++) {
          const offset = i * 4;
          const [expectedR, expectedG, expectedB] = invertRgbExpected(
            data[offset],
          ...
          expect(dataInv[offset + 1]).toEqual(expectedG);
          expect(dataInv[offset + 2]).toEqual(expectedB);
          expect(dataInv[offset + 3]).toEqual(data[offset + 3]);
        }
      }
